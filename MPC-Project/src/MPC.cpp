...
public:
	...
	void operator()(ADvector& fg, const ADvector& vars) {
		...
		for(int i = 0; i<N; i++) {
			...
			+ 200*CppAD::pow(vars[epsi_start + i] - ref_epsi,2)
		...
		for(int i = 0; i<N-2; i++) {
			...
			+ 10*CppAD::pow(vars[a_start + i] - vars[a_start + i +1],2);
		}
		...
		for (int i = 0; i < N - 1; i++) {
			...
			//a = vars[a_start + i];
			if (i > int(.01/dt)) delta = vars[delta_start + i - int(.01/dt)];
			else delta = prevDelta;
			if (i > int(.01/dt)) a = vars[a_start + i - int(.01/dt)];
			else a = prevThr;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
	...
	return {solution.x[delta_start], solution.x[a_start],
		solution.x[x_start], solution.x[x_start+1],solution.x[x_start+2],
		solution.x[x_start+3],solution.x[x_start+4],
		solution.x[y_start],solution.x[y_start+1],solution.x[y_start+2],
