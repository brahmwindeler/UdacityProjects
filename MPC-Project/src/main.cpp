    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for(int i = 0; i<ptsx.size(); i++) ptsxVals[i] = (ptsx[i] - px)*cos(psi) + (ptsy[i] - py)*sin(psi);
          Eigen::VectorXd ptsyVals = Eigen::VectorXd(ptsy.size());
          for(int i = 0; i<ptsy.size(); i++) ptsyVals[i] = (ptsy[i]- py)*cos(psi) - (ptsx[i] - px)*sin(psi);
          auto coeffs = polyfit(ptsxVals, ptsyVals, 3);
          ...
          */
          ...
          state << 0, 0, 0, v, cte, epsi, -steer, thr;
